...
def remove_from_cart(request, pk):
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            order_item.delete()
            messages.warning(request, "This item removed from your cart")
            return redirect("App_Order:cart")
        else:
            messages.info(request, "This item is not in your cart.")
    else:
        messages.info(request, "You dont hane any active order")
        return redirect("App_Shop:home")
...
def increase_cart(request, pk):
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            messages.info(request, f"{item.name} doesn't exist in your cart")
            return redirect("App_Shop:home")
    else:
