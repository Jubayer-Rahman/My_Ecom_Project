...
def payment(request):
    ...
    if not saved_address.is_fully_filled():
        messages.info(request, "Please fillup your shipping adress")
    ...
    store_id = 'advan60e2e19c1fa7e'
    API_key = 'advan60e2e19c1fa7e@ssl'
    ...
    order_total = order_qs[0].get_totals()
    mypayment.set_product_integration(total_amount=Decimal(order_total), currency='BDT', product_category='Mixed', product_name=order_items, num_of_item=order_items_count, shipping_method='Pickup', product_profile='None')

    current_user = request.user
    mypayment.set_customer_info(name=current_user.profile.full_name, email=current_user.email, address1='None', address2='None', city='None', postcode='None', country='None', phone=current_user.profile.phone)

    mypayment.set_shipping_info(shipping_to=current_user.profile.full_name, address=saved_address.address, city=saved_address.city, postcode='None', country=saved_address.country)
